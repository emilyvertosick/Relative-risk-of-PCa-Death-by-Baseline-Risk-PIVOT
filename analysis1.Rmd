...
df_analysis_gam <-
  ...
  mutate(
    ...
    # Create logistic GAM model for outcome within 15 years
    logistic_gam_model =
      map(
        df_model,
        ~ mgcv::gam(
          out15 ~ s(preopbcr_xb, bs = "cs"),
          family = "binomial",
          data = ..1,
          method = "REML"
        )
    ...
    cox_gam_pred =
      map2(
        cox_gam_model, df_model,
        ~ predict(
          ..1,
          newdata = ..2 %>% mutate(out = 0, ttout = 15),
          se.fit = TRUE,
          type = "response"
...
df_gam_final <-
  ...
  # Nest separately by outcome
  nest(df_gam_pred = -c(outcome)) %>%
  mutate(
    label =
      case_when(
    ...
    gg_cox_gam =
      map2(
        ...
        ~ ggplot() +
          ...
          labs(
            x = "Risk of BCR within 5 Years",
            y = as.character(str_glue("Risk of {..2} within 15 Years"))
          ) +
...
df_gam_graphdata <-
  ...
  select(outcome, df_cox_gam, df_logistic_gam)
  ...
  mutate(
    ...
    cox_pred =
      map(
        ...
        ~ predict(
          ..1,
          newdata = tibble(preopbcr_pr5 = seq(0, 1, length.out = 100), out = 0, ttout = 15),
          se.fit = TRUE,
          type = "response"
        ) %>% as_tibble()
      ),
    ...
    cox_pred =
      map(
        ...
        ~ bind_cols(
          ..1,
          tibble(preopbcr_pr5 = seq(0, 1, length.out = 100), out = 0, ttout = 15)
        ) %>%
...
df_bs_pred <-
  df_bs_analysis %>%
  select(boot, rp, outcome, cox_pred) %>%
  unnest(cox_pred) %>%
  # Reshape to wide to calculate difference between immediate RP and delayed RP
  select(boot, rp, outcome, preopbcr_pr5, ttout, event_pr) %>%
  pivot_wider(
    names_from = "rp",
...
df_bs_final <-
  ...
  nest(df_pred = -c(outcome, preopbcr_pr5)) %>%
  ...
  select(outcome, preopbcr_pr5, event_pr_diff, lb, ub) %>%
  nest(df_plot = -c(outcome))
